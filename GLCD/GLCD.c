...
void LCD_Initialization(void)
{
	...
	if( DeviceCode == 0x9325 || DeviceCode == 0x9328 )	
	{
		...
		LCD_WriteReg(0x0003,0x1030); 	/* Entry Mode Set*/ // LCD_WriteReg(0x0003,0x1018);
...
void LCD_Clear(uint16_t Color)
{
	...
	{	
		LCD_SetCursor(0,0); // LCD_SetCursor(0,319)
	}	
/*
...
void PutChar2( uint16_t Xpos, uint16_t Ypos, uint8_t ASCI, uint16_t charColor, uint16_t bkColor )
{
	uint16_t i, j;
    uint8_t buffer[16], tmp_char;
		uint16_t BitMap[16*8];
    GetASCIICode(buffer,ASCI);  /* 取字模数据 */
    for( i=0; i<16; i++ )
    {
        tmp_char = buffer[i];
        for( j=0; j<8; j++ )
        {
            if( ((tmp_char >> (7 - j)) & 0x01) == 0x01 )
            {
								BitMap[i * 8 + j] = charColor;	
            }
            else
            {
								BitMap[i * 8 + j] = bkColor;
            }
        }
    }
		
		LCD_SetWindow(Xpos, Ypos, 8, 16);
		LCD_CS(0);
		LCD_WriteCommand(0x22);
		LCD_RS(1);
		
		for( i = 0; i < 16 * 8; i++){
			LCD_WritePixel(BitMap[i]);
		}
		LCD_CS(1);
}
...
void GUI_Text(uint16_t Xpos, uint16_t Ypos, uint8_t *str,uint16_t Color, uint16_t bkColor)
{
    ...
    {
        TempChar = *str++;  
        PutChar2( Xpos, Ypos, TempChar, Color, bkColor );    
        if( Xpos < MAX_X - 8 )
        {
            Xpos += 8;
        } 
